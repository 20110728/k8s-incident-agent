...
def readiness_configuration_facts(container: dict, expected) -> dict:
    ...
    An explicit null is the collector's observation of no HTTP probe. A missing
    field or incomplete dict is insufficient evidence, never confirmed agreement.
    """
    result = dict(
        readiness_configuration_status="unknown",
        readiness_drift=False,
        readiness_patch_supported=False,
    )
    if "readiness_probe" not in container:
        return result
    probe = container["readiness_probe"]
    if probe is None:
        result.update(readiness_configuration_status="drift", readiness_drift=True)
        return result
    if (
        not isinstance(probe, dict)
        or not probe.get("path")
        or probe.get("port") is None
    ):
        return result
    same_scheme = (probe.get("scheme") or "HTTP") == expected.scheme
    same_address = (probe["path"], probe["port"]) == (expected.path, expected.port)
    drift = not (same_scheme and same_address)
    result.update(
        readiness_configuration_status="drift" if drift else "matched",
        readiness_drift=drift,
        readiness_patch_supported=same_scheme and not same_address,
    )
    return result
...
def diagnostic_facts(state: dict) -> dict:
    ...
    facts = dict(
        ...
        current_runtime_faults=[],
        configuration_evidence_complete=False,
        current_log_evidence_ids=[],
        profile_matched=False,
        readiness_configuration_status="unknown",
        readiness_patch_supported=False,
        expected_replicas=None,
        observed_desired_replicas=None,
        replica_count_matches=None,
    )
    try:
    ...
    service_valid = (
        svc.get("namespace") == ns
        and svc.get("name") == service
        and isinstance(svc.get("selector"), dict)
    )
    if service_valid:
        facts["selector_drift"] = (
            isinstance(svc.get("selector"), dict)
    ...
    facts["configuration_evidence_complete"] = bool(
        service_valid
        and container
        and probe.get("path")
        and probe.get("port") is not None
    )
    facts.update(readiness_configuration_facts(container, profile.readiness_probe))

    # 只评价登记 Deployment 拥有的 Pod，避免 selector 错误时混入其他工作负载。
    owned = {
        e["resource_name"]
        for e in items("OwnerChain")
        if e.get("data", {}).get("owner_chain", {}).get("deployment_name")
        == profile.deployment_name
        and e["data"]["owner_chain"].get("namespace") == ns
    }
    pods = [
        e
        for e in items("PodStatus")
        if e["resource_name"] in owned and e["data"].get("namespace") == ns
    ]
    endpoints = [
        e
        for e in items("EndpointSlice")
        if e["data"].get("namespace") == ns and e["data"].get("service_name") == service
    ]
    facts["resource_evidence_ids"] = [
        e["evidence_id"] for e in [dep_e, *pods, *endpoints]
    ]
    for pod in pods:
        for c in pod["data"].get("containers", []):
            ...
            reason = (
                ...
                if c.get("state") == "waiting"
                else (
                    c.get("terminated_reason")
    ...
    facts["current_log_evidence_ids"] = [
        e["evidence_id"]
        for e in items("PodLogs")
        if e["resource_name"] in owned
        and e["data"].get("previous") is False
        and bool(e["data"].get("content"))
    ]
    desired = dep.get("desired_replicas")
    # A live scale-down must not redefine an explicitly registered expectation.
    # Legacy profiles without that contract still use their observed replica count.
    facts["expected_replicas"] = profile.expected_replicas
    facts["observed_desired_replicas"] = desired
    if profile.expected_replicas is not None and type(desired) is int:
        facts["replica_count_matches"] = desired == profile.expected_replicas
    if type(desired) is int and desired >= 0:
        if (
            desired == 0
            or facts["replica_count_matches"] is False
            or dep.get("ready_replicas", 0) < desired
            or dep.get("available_replicas", 0) < desired
            or facts["selector_drift"]
            or facts["readiness_drift"]
            or any(p["data"].get("ready") is False for p in pods)
        ):
            ...
            and all(p["data"].get("ready") is True for p in pods)
            and any(
                ep.get("ready") is True and ep.get("target_name") in owned
                for e in endpoints
                for ep in e["data"].get("endpoints", [])
            )
        ):
    ...
    for check in profile.business_checks:
        ...
        try:
            ...
            valid = (
                data.get("scope") == "cluster_service_http"
                and data.get("request_id")
                and e.get("source") == "cluster_http_probe"
                and json.dumps(data.get("target"), sort_keys=True)
        ...
        if status == "passed" and not (
            data.get("http_status") == check.expected_status
            and data.get("content_matches") is True
            and data.get("error_code") is None
        ):
            status = "unknown"
...
def validate_diagnosis_assessment(diagnosis: Diagnosis, state: dict) -> None:
    ...
    if a is None:
        raise InvalidDiagnosisAssessment(
            "new diagnosis requires assessment schema_version=v2"
        )
    ...
    require(
        a.resource_status == facts["resource_status"],
        "resource_status must equal policy_facts.resource_status",
    )
    require(
        a.business_status == facts["business_status"],
        "business_status must equal policy_facts.business_status",
    )
    for finding in [*a.symptoms, *a.root_cause_hypotheses]:
        require(
            set(finding.evidence_ids) <= set(diagnosis.evidence_ids),
            "finding references must be declared in diagnosis.evidence_ids",
        )
    require(
        set(facts["business_evidence_ids"]) <= set(diagnosis.evidence_ids),
    ...
    if category == "no_fault_detected":
        ...
        require(
            set(facts["resource_evidence_ids"]) <= set(diagnosis.evidence_ids),
            "no_fault_detected must cite current resource evidence",
        )
        require(
            not a.root_cause_hypotheses,
            "no_fault_detected must not assert an active root cause",
        )
        ...
        if not grounded_configuration:
            require(
                bool(a.missing_evidence) and bool(a.next_investigation),
    ...
    if category in {"service_selector_mismatch", "readiness_probe_error"}:
        ...
        require(
            ...
            "configuration diagnosis must cite Service and registered Deployment",
        )
        # A top-level citation does not bind that evidence to a supported claim.
        # Surface missing claim references during diagnosis, while its existing
        # validation retry can still correct them; never attach references here.
        required_ids = set(facts["configuration_evidence_ids"])
        missing_by_hypothesis = [
            {
                "hypothesis_index": index,
                "missing_evidence_ids": sorted(required_ids - set(h.evidence_ids)),
            }
            for index, h in enumerate(a.root_cause_hypotheses)
            if h.status == "supported" and required_ids - set(h.evidence_ids)
        ]
        require(
            not missing_by_hypothesis,
            "SUPPORTED_CONFIGURATION_REFERENCES_INCOMPLETE: each supported configuration "
            "hypothesis must itself cite Service and registered Deployment evidence; "
            f"missing={missing_by_hypothesis!r}. Reassess the evidence: cite it only if it "
            "supports this claim; otherwise keep the hypothesis suspected and state "
            "the genuine missing evidence. Top-level evidence_ids alone are insufficient.",
        )
    if category == "application_error":
        supported = facts["business_status"] == "failed" or bool(
            facts["current_runtime_faults"]
        )
        require(
            supported,
            "application_error needs a current observed failure, not connection error alone",
        )
    if category in {"crash_loop_backoff", "image_pull_backoff", "oom_killed"}:
        require(
            category in facts["current_runtime_faults"],
            "historical logs/events/restarts cannot establish a current runtime failure",
        )
    if category == "dependency_error":
        require(
            facts["resource_status"] == "not_ready"
            or facts["business_status"] == "failed",
            "dependency hypothesis needs current failure symptoms",
        )
        require(
            bool(set(facts["current_log_evidence_ids"]) & set(diagnosis.evidence_ids)),
            "dependency hypothesis needs current workload log evidence; connection error alone is insufficient",
        )
        require(
            bool(a.root_cause_hypotheses),
            "dependency_error is a suspected cause and requires explicit hypotheses",
        )
    ...
    if category not in {"service_selector_mismatch", "readiness_probe_error"}:
        require(
            all(h.status == "suspected" for h in a.root_cause_hypotheses),
            "underlying runtime/dependency causes remain suspected in this evidence scope",
        )
...
def can_report_insufficient_evidence(facts: dict) -> bool:
    return bool(
        ...
        and not facts["readiness_drift"]
        and not facts["current_runtime_faults"]
        and facts["business_evidence_ids"]
        and facts["resource_evidence_ids"]
    )
...
def insufficient_evidence_diagnosis(state: dict) -> CurrentDiagnosis:
    ...
    if not can_report_insufficient_evidence(facts):
        raise InvalidDiagnosisAssessment(
            "insufficient-evidence precheck is not applicable"
        )
    refs = list(
        dict.fromkeys(facts["resource_evidence_ids"] + facts["business_evidence_ids"])
    )
    return CurrentDiagnosis(
        ...
        assessment=dict(
            ...
            symptoms=[
                dict(
                    summary="当前采样显示登记工作负载及关联资源未就绪。",
                    evidence_ids=facts["resource_evidence_ids"],
                ),
                dict(
                    summary="登记业务检查结果未知，未证明接口正常，也未取得可用于确定业务根因的结果。",
                    evidence_ids=facts["business_evidence_ids"],
                ),
            ],
            root_cause_hypotheses=[],
            missing_evidence=[
                "与本次故障时间对应的应用诊断信息及就绪失败原因",
                "实际依赖关系及独立下游检查结果",
            ],
            next_investigation=[
                "由登记负责人核对应用就绪检查的失败原因，并结合当前日志定位所缺证据",
