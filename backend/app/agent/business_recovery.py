...
class BusinessRecoveryVerifier:
    ...
    def verify(self, state) -> RecoveryVerificationResult:
        ...
        data.update(
            ...
            resource_status='unknown', business_status='skipped',
            unverified_scope=list(UNVERIFIED_SCOPE),
            post_repair_evidence=[], post_repair_profile=None,
        ...
        try:
            ...
            bundle = self.collector.collect(request['namespace'], request['service_name'])
            ...
            data.update(post_repair_evidence=evidence, post_repair_profile=snapshot,
            ...
            if action.get('action') == 'patch_readiness_probe' and action.get('applied_patch'):
                expected_generation = generation + 1 if type(generation) is int else None
            if (snapshot.get('status') != 'matched'
                    or not original.get('digest') or not original.get('deployment_uid')
            ...
            resource_ok = (facts['resource_status'] == 'ready'
            ...
            data['checks'].extend([
                VerificationCheck(name='post_repair_resources', passed=resource_ok,
                    observed=facts['resource_status'], expected='ready without current drift/fault',
                    message='处置后重新采集的登记资源状态。').model_dump(mode='json'),
                VerificationCheck(name='registered_business_checks', passed=business_ok,
                    observed=facts['business_status'], expected='passed',
                    message='仅评价本次登记 Service 接口检查，不覆盖全部业务。').model_dump(mode='json'),
            ])
            success = resource_ok and business_ok
            data['status'] = 'succeeded' if success else 'failed'
            data['error_code'] = (None if success else 'POST_REPAIR_RESOURCES_NOT_READY'
                                  if not resource_ok else 'POST_REPAIR_BUSINESS_FAILED'
                                  if facts['business_status'] == 'failed' else 'POST_REPAIR_BUSINESS_UNKNOWN')
            data['message'] = ('资源复查及登记业务检查通过；仅限本次采样范围。' if success
                               else '未确认恢复：请分别查看资源复查和登记业务检查结果。')
            data['error_message'] = None if success else data['message']
        except Exception as error:
            # 包括检查器异常、资源变化或无法绑定目标；失败关闭但保留已采集证据。
            data.update(status='failed', resource_status='unknown', business_status='unknown',
